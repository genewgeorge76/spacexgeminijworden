...
export default function EstimatePricePanel({ result, serviceLabel }: EstimatePricePanelProps) {
  ...
  const tierColors = {
    ...
    fish: { bg: 'bg-zinc-500/20', border: 'border-zinc-500', text: 'text-zinc-400' },
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-zinc-900 border-2 border-[#ffcc00] p-6">
        ...
        {/* State + multiplier */}
        <div className="flex gap-4 text-xs font-bold text-zinc-500 border-t border-zinc-800 pt-3">
      ...
      <div className="bg-zinc-900 border border-zinc-700">
        ...
        <div className="bg-zinc-800/50 px-4 py-3 border-t border-zinc-700">
          <div className="flex justify-between text-sm">
            <span className="text-zinc-400 font-bold">Subtotal (Materials + Labor)</span>
            <span className="text-white font-bold">
              ${estimate.totalShielded.toLocaleString('en-US', { maximumFractionDigits: 0 })}
            </span>
          </div>
          <div className="flex justify-between text-sm mt-1">
            <span className="text-zinc-400 font-bold">Worden Margin ({estimate.profitTargetPct}%)</span>
            <span className="text-white font-bold">
      ...
      <div className="bg-zinc-900 border border-zinc-700 p-4">
        ...
        <div className="mt-3 pt-3 border-t border-zinc-800 grid grid-cols-3 gap-3 text-center">
          <div>
            ...
            <p className="text-zinc-600 text-[10px] uppercase tracking-wider font-bold">Marshall Compaction</p>
          </div>
          <div>
            <p className="text-[#ffcc00] font-black text-lg">{SOVEREIGN_BASE_DEPTH_IN}"</p>
            <p className="text-zinc-600 text-[10px] uppercase tracking-wider font-bold">Stone Base Depth</p>
          </div>
          <div>
            <p className="text-[#ffcc00] font-black text-lg">±${OIL_SHIELD_BUFFER_PER_TON}</p>
            <p className="text-zinc-600 text-[10px] uppercase tracking-wider font-bold">Oil Shield/Ton</p>
          </div>
        </div>
      </div>
    </div>
