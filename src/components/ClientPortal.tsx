  ...
  return (
    <section className="bg-[#111111] p-8 rounded-lg mt-10 border border-gray-800">
      <div className="mb-6">
        ...
        {client.clientName && (
          <p className="text-gray-400 text-sm mt-1">Client: {client.clientName}</p>
        )}
        <p className="text-gray-500 text-xs mt-1">{report.logic}</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-black p-4 rounded border-l-4 border-[#ffcc00]">
          <h4 className="text-gray-400 text-xs uppercase tracking-widest mb-1">PCI Score</h4>
          <p className="text-[#ffcc00] text-3xl font-bold">{report.pciScore}</p>
          <p className="text-gray-500 text-xs mt-1">Pavement Condition Index</p>
        </div>

        <div className={`bg-black p-4 rounded border-l-4 ${isAlert ? 'border-red-500' : 'border-green-500'}`}>
          <h4 className="text-gray-400 text-xs uppercase tracking-widest mb-1">Maintenance Status</h4>
          <p className={`text-sm font-bold mt-1 ${isAlert ? 'text-red-400' : 'text-green-400'}`}>
            {report.maintenanceAlert}
          </p>
          <p className="text-gray-500 text-xs mt-1">Last service: {client.lastServiceDate} months ago</p>
        </div>

        <div className="bg-black p-4 rounded border-l-4 border-[#ffcc00]">
          <h4 className="text-gray-400 text-xs uppercase tracking-widest mb-1">Estimated Repair Value</h4>
          ...
          <p className="text-gray-500 text-xs mt-1">{client.squareFootage.toLocaleString()} sq ft analyzed</p>
        </div>
      </div>
