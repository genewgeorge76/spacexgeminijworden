  ...
  return (
    <div className="space-y-8">
            ...
            <tbody>
              {livePrices.map((m) => (
                <tr key={m.id} className="border-b border-gray-900 hover:bg-gray-900/30">
                  ...
                  <td className="py-2 pr-4 text-gray-500 font-mono">{m.spec}</td>
                  <td className="py-2 pr-4 text-right text-gray-400">${m.baseline.toFixed(2)}/{m.unit}</td>
                  ...
                  <td className="py-2 pr-4 text-right">
                    <span className={`flex items-center justify-end gap-1 font-bold ${m.delta > 0 ? 'text-orange-400' : m.delta < 0 ? 'text-green-400' : 'text-gray-500'}`}>
                  ...
                  <td className="py-2 text-right">
                    {m.shieldActive
                      ...
                      : <span className="text-gray-500">{m.unit === 'gal' ? `$${m.shieldedPrice.toFixed(2)}/gal` : 'N/A'}</span>
                    }
                  </td>
                </tr>
              ))}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div>
          <h3 className="text-sm font-black uppercase tracking-wider text-gray-400 mb-4">Project Parameters</h3>
          <div className="space-y-4 bg-[#111] border border-gray-800 p-5">
            {/* State */}
            <div>
              <label className="text-xs font-black uppercase text-gray-500 mb-1 block">State (50-State Coverage)</label>
              <select
                value={stateCode}
            ...
            {/* Service Type */}
            <div>
              <label className="text-xs font-black uppercase text-gray-500 mb-1 block">Service Type</label>
              <select
                value={serviceType}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-black uppercase text-gray-500 mb-1 block">Length (ft)</label>
                <input type="number" value={lengthFt} min={1} onChange={(e) => setLengthFt(Number(e.target.value))}
                  className="w-full bg-gray-900 border border-gray-700 text-white text-sm p-2 font-bold focus:border-[#ffcc00] outline-none" />
              </div>
              <div>
                <label className="text-xs font-black uppercase text-gray-500 mb-1 block">Width (ft)</label>
            ...
            {/* Surface depth */}
            <div>
              <label className="text-xs font-black uppercase text-gray-500 mb-1 block">Surface Course Depth (in)</label>
        ...
        {/* Bid Output */}
        <div>
          <h3 className="text-sm font-black uppercase tracking-wider text-gray-400 mb-4">Live Bid Output</h3>
          <div className="bg-[#111] border border-[#ffcc00]/40 p-5 space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className="text-xs text-gray-500 uppercase font-bold">Area</div>
                <div className="text-2xl font-black text-white">{estimate.sqFt.toLocaleString()} <span className="text-sm text-gray-400">sq ft</span></div>
              </div>
              <div>
                <div className="text-xs text-gray-500 uppercase font-bold">Final Bid</div>
                <div className="text-2xl font-black text-[#ffcc00]">{fmt(estimate.finalBidPrice)}</div>
                <div className="text-xs text-gray-500">${estimate.pricePerSqFt}/sq ft</div>
              </div>
            </div>
            ...
            <div className="border-t border-gray-800 pt-3 space-y-1 text-xs">
              ...
              <div className="flex justify-between border-t border-gray-800 pt-2 mt-2">
                <span className="text-gray-500 font-bold uppercase text-xs">Materials + Labor</span>
                <span className="text-white font-black">{fmt(estimate.totalShielded)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500 font-bold uppercase text-xs">Profit ({estimate.profitTargetPct}% margin)</span>
