...
const locationsDir = path.join(__dirname, 'src', 'routes', 'locations');
const files = fs.readdirSync(locationsDir).filter(f => f.endsWith('.tsx'));

for (const file of files) {
  const filePath = path.join(locationsDir, file);
  let content = fs.readFileSync(filePath, 'utf8');
  
  // Extract location name from filename (e.g. chesterfield.tsx -> Chesterfield)
  const locName = file.replace('.tsx', '').split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
  const routeName = file.replace('.tsx', '');
  
  const newContent = `import { createFileRoute, Link } from '@tanstack/react-router';
import { MapPin, Shield, HardHat, Phone } from 'lucide-react';

export const Route = createFileRoute('/locations/${routeName}')({
  component: () => (
      ...
      <section className="relative py-32 px-6 lg:px-12 bg-[#1a1a1a] border-b-[8px] border-[#ffcc00] overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10 text-center">
          <span className="bg-[#ffcc00] text-black px-6 py-2 font-black uppercase text-xs tracking-[0.3em] mb-8 inline-block shadow-lg">
            Local Paving Experts in ${locName}, VA
          </span>
          ...
          <p className="text-2xl text-zinc-400 italic font-bold max-w-3xl mx-auto leading-relaxed">
            Premium residential and commercial paving solutions in ${locName}, engineered with our signature 6-inch structural stone base for unmatched durability.
          </p>
      ...
      <section className="py-24 px-6 lg:px-12 bg-black">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-6xl font-black uppercase text-white tracking-tighter mb-4">Services in <span className="text-[#ffcc00]">${locName}</span></h2>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="bg-[#1a1a1a] p-8 border border-zinc-800 hover:border-[#ffcc00]/50 transition-all shadow-2xl">
              ...
              <p className="text-zinc-400 leading-relaxed mb-6">
                From luxury estate driveways to standard residential replacements, we provide ${locName} homeowners with paving that boosts curb appeal and property value.
              </p>
            ...
            <div className="bg-[#1a1a1a] p-8 border border-zinc-800 hover:border-[#ffcc00]/50 transition-all shadow-2xl">
              ...
              <p className="text-zinc-400 leading-relaxed mb-6">
                We manage heavy-traffic commercial paving in ${locName}, including ADA compliance, structural repairs, and fast-track QSR lot replacements.
              </p>
            ...
            <div className="bg-[#1a1a1a] p-8 border border-zinc-800 hover:border-[#ffcc00]/50 transition-all shadow-2xl">
              ...
              <p className="text-zinc-400 leading-relaxed mb-6">
                Protect your asphalt investment or opt for a rustic tar and chip finish. Our ${locName} maintenance team ensures your surfaces last decades.
              </p>
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-16 items-center">
          <div className="lg:w-1/2">
            <h2 className="text-4xl md:text-5xl font-black uppercase text-white tracking-tighter mb-6">
              Why ${locName} Chooses <br/><span className="text-[#ffcc00]">J. Worden & Sons</span>
            </h2>
            <div className="space-y-6 text-lg text-zinc-300">
              <p>For over three generations, we've been the trusted asphalt contractor for the ${locName} area and the greater 41-city grid.</p>
          ...
          <div className="lg:w-1/2 bg-black p-10 border-l-[8px] border-[#ffcc00] shadow-2xl">
             <h3 className="text-3xl font-black uppercase text-white mb-4">Need an Estimate in ${locName}?</h3>
...
console.log('Location pages enhanced successfully!');
